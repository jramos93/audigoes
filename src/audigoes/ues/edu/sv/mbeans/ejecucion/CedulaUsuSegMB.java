...
@ManagedBean(name = "ceduUsuSegMB")
@ViewScoped
public class CedulaUsuSegMB extends AudigoesController implements Serializable {
	/**
	 * 
	 */
...
//					new Object[] { auditoria.getAudId() }));
			...
			setListado((List<CedulaNota>) audigoesLocal.findByNamedQuery(CedulaNota.class, "notas.by.usuario.seguimiento",
					new Object[] { getObjAppsSession().getUsuario().getUsuId() }));
			System.out.println(" "+getObjAppsSession().getUsuario().getUsuId());
		} catch (Exception e) {
			e.printStackTrace();
